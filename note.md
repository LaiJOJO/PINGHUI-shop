...
--------------接下来不进行vuex处理请求，直接和data对接
 ...
 5.9 完善功能
 a. 导航首守卫完善 ： 
 1. 结算界面没登陆不能跳转 , 路径包含pay和trade的页面不能跳转，先登录再跳转 ；给当前login路径添加query参数来保存点击前的路径，一般用?redirect = to.path保存，点击登陆后能获取query参数判断是跳转去home还是其他页面
 2. 独享守卫设置不能跳转成功支付的页面,只能从pay路径进行跳转 ; pay只能从结算订单页面跳转

 b. 图片懒加载 : vue - lazyload 包 

 c. 表单校验插件:vee-validate :
 1. 注意版本安装npm包,这里使用2版本的，3版本的后期试试
 2. computed的值是不能修改的，执行了context的vuex仍会向下执行，利用异步进行返回值获取
 3. await this.$validator.validateAll() 会返回bool值，根据是否全部验证通过返回

 d. 路由懒加载: 利用动态导入，用到组件再用函数import

 e. 关闭打包时的map文件，vue.config下加入配置 : productionSourceMap:false 