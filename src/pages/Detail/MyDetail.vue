        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="eachAttr in spuSaleAttrList" :key="eachAttr.id">
                <dt class="title">{{ eachAttr.saleAttrName }}</dt>
                <dd changepirce="0" style="cursor:pointer;" :class="item.isChecked == '1' ? 'active' : ''"
                  v-for="item in eachAttr.spuSaleAttrValueList" :key="item.id"
                  @click="addIsChecked(item, eachAttr.spuSaleAttrValueList)">
        ...
        <div class="fitting">
          ...
          <div class="good-suits">
            ...
            <div class="result">
              ...
              <div class="price">￥{{ detailList.price }}</div>
...
export default {
  ...
  mounted() {
    // 发布事件
    this.$store.dispatch('detail/getDatailList', this.$route.params.skuId)
  },

  computed: {
    ...mapGetters({ categoryView: 'detail/categoryView', skuInfo: 'detail/skuInfo', spuSaleAttrList: 'detail/spuSaleAttrList' }),
    ...mapState({ detailList: (state) => { return state.detail.detailList } }),
  },

  methods: {
    // 右侧商品属性点击高亮
    addIsChecked(curAttr, allAttr) {
      allAttr.forEach(eachAttr => eachAttr.isChecked = 0)
      // 有点击切换属性就将属性传递过去，没有就直接用默认版本
      this.mySkuAttr = this.spuSaleAttrList
      curAttr.isChecked = 1
    },
    // 输入框值判断修改,正则判断
    getGoodAmount(){
